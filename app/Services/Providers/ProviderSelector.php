...
final class ProviderSelector implements ProviderSelectorContract
{
    ...
    private function isHealthyEnough(ServiceProvider $provider): bool
    { return in_array($provider->health_status ?? 'unknown', ['unknown', 'online', 'slow'], true); }

    private function supports(ServiceProvider $provider, ServiceProviderOperation $operation): bool
    {
        $capability = $provider->capabilities()->where('operation', $operation->value)->first();
        return $capability === null || (bool) $capability->supported;
    }

    private function withinCapacity(ServiceProvider $provider): bool
    {
        if ($provider->max_services !== null && $provider->services()->count() >= (int) $provider->max_services) return false;
        if ($provider->max_users !== null && $provider->services()->distinct('telegram_account_id')->count('telegram_account_id') >= (int) $provider->max_users) return false;
        return true;
    }
    ...
    private function withinPolicy(ServiceProvider $provider, ServiceProviderOperation $operation, TelegramAccount $account, Plan $plan): bool
    {
        ...
        if ($policy->max_services_per_user !== null && $provider->services()->where('telegram_account_id', $account->getKey())->count() >= (int) $policy->max_services_per_user) return false;
        return true;
    }
}
